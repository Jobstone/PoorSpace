...
public class PoorSpace extends JavaPlugin {
	...
	private void filesUpdate() {
		...
		if (chunks.exists()) {
			Bukkit.getConsoleSender().sendMessage("启动文件更新");
			File spaces = new File(this.getDataFolder(), "spaces");
			chunks.renameTo(spaces);
			for (File worldFile : spaces.listFiles()) {

				Bukkit.getConsoleSender().sendMessage("更新世界文件："+worldFile.getName());
				for (File spaceFile : worldFile.listFiles()) {
					FileConfiguration config = YamlConfiguration.loadConfiguration(spaceFile);
					if (config.contains("permission1")) {
						...
						Bukkit.getConsoleSender().sendMessage("文件出现问题："+spaceFile.getName());
				}
